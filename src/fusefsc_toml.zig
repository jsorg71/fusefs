...
//*****************************************************************************
fn load_fusefsc_config(file_name: []const u8) !*c.toml_table_t
{
...
//*****************************************************************************
pub fn setup_fusefsc_info(allocator: *const std.mem.Allocator,
        info: *fusefsc.fusefsc_info_t, config_file: []const u8) !void
{
    try log.logln(log.LogLevel.info, @src(),
            "config file [{s}]", .{config_file});
    g_allocator = allocator;
    const table = try load_fusefsc_config(config_file);
    defer c.toml_free(table);
    try log.logln(log.LogLevel.info, @src(),
            "load_fusefsc_config ok for file [{s}]",
            .{config_file});
    var index: c_int = 0;
